...
- 主数据主表的 `DocEntry` 主键与 `Code` 唯一键。两者应与业务对象模板一致，不能把 `Code` 同时改成主键。
- 子表/孙表关系。解析器创建的 Excel 子表关系能力有限，复杂层级不能仅凭成功日志判断。

发现问题时，把临时汇总 XML 交给 `generating-ava-cloud-business-objects` 的“校验转换结果”模式。由模型阶段做最小修正并记录；不要修改用户原 Excel，除非用户明确要求回写。
