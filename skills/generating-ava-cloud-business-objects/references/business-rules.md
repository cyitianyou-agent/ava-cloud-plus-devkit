...
依次采用：用户本次明确要求、当前会话已确认信息、更新任务的目标 XML、内置模板和规则、默认推断。低优先级资料不得覆盖高优先级的明确值。

目标 XML 只用于更新定位、重复检查和保留原值，不用于推断模块简称、输出位置或新建对象惯例。`${Company}`、`Date/Default`、`Numeric/Quantity` 等历史写法在更新中保留，在新建中不照抄。

创建任务不扫描现有模块对象、源码或固定目录。模块简称只来自当前会话；输出位置只依据当前会话判断，无法可靠确定时询问用户。

新建对象或新增模型时，把模块简称作为生成前置条件。当前会话未提供或存在冲突时，当前轮次只询问模块简称并停止，不进行项目检索，也不给出候选简称。模块简称满足后再判断输出位置。
...
- 新建采用 `AVA_AA_BBB`。
- `AVA` 固定。
- `AA` 使用当前会话中用户明确给出或已经确认的模块简称，长度为 1–5；同时写入 `Domain.ShortName`。
...
- [ ] 新建或新增模型所需的模块简称来自当前会话；未提供时已经询问用户。
- [ ] 输出位置来自会话明确指定或根据会话可靠判断；无法判断时已经询问用户。
- [ ] 对象类型有明确语义依据。
- [ ] 所有主表和行表完整复制对应模板字段。
- [ ] 主键、唯一键和孙表 `ItemId` 正确。
